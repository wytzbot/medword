...
export async function requestWords(category,level){
 ...
 // Ask the AI on every level when possible. This is the important part: the
 // previous implementation only called AI after the local pool became small,
 // so a tiny cached bank could be replayed for many levels even though Groq
 // was healthy. We now ask for genuinely fresh candidates every level.
 const exclude=[...new Set(recentList)].slice(0,120);
 const terms=await fetchAiTerms(category,lvl,exclude);
 if(terms?.length){
  const novel=terms.filter(w=>!recentSet.has(w.word));
  const merged=cleanTerms([...novel,...cached],category);
  store.setTerms(category,merged);
  cached=merged;
 }
 let fresh=cached.filter(x=>!recentSet.has(x.word));
 if(fresh.length>=n)return pick(fresh,category,lvl);
 if(fresh.length>=4)return pick(fresh,category,lvl);
 // If AI returned too little, make one more request excluding the whole local
 // cache. This gives the model a chance to create a new batch rather than
 // selecting the same legacy words again.
 const secondExclude=[...new Set([...recentList,...cached.map(w=>w.word)])].slice(0,150);
 const second=await fetchAiTerms(category,lvl,secondExclude);
 if(second?.length){
  const secondFresh=second.filter(w=>!recentSet.has(w.word));
  if(secondFresh.length){
   cached=cleanTerms([...secondFresh,...cached],category);
   store.setTerms(category,cached);
   fresh=cached.filter(x=>!recentSet.has(x.word));
  }
 }
 if(fresh.length>=4)return pick(fresh,category,lvl);
 const arr=fallback[category]||[];
 const fallbackPool=arr.map(word=>({word,definition:'Medical learning term.',category})).filter(x=>!recentSet.has(x.word));
 return pick(fallbackPool.length?fallbackPool:arr.map(word=>({word,definition:'Medical learning term.',category})),category,lvl);
}
