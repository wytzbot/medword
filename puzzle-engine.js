export class Game{
 constructor(words,level,{pro=false}={}){
  ...
  const seen=new Set();this.words=cleaned.filter(x=>{if(seen.has(x.word))return false;seen.add(x.word);return true}).slice(0,12);
  if(this.words.length<6)throw new Error('Not enough valid unique terms');
  this.level=level;this.found=new Set;this.onUpdate=()=>{};const longest=Math.max(...this.words.map(x=>x.word.length));this.size=Math.min(18,Math.max(10,10+Math.floor(level/20),longest));
