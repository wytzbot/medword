const SEL_COLORS=['rgba(79,70,229,.28)','rgba(74,222,128,.32)','rgba(96,165,250,.32)','rgba(248,113,113,.32)','rgba(250,204,21,.35)','rgba(251,146,60,.32)','rgba(255,255,255,.55)'];
export class Game{
 ...
 build(){
  ...
  // Higher levels lean harder on making new words cross already-placed ones,
  // which packs the grid tighter and makes scanning for terms more demanding.
  const crossBias=Math.min(.92,.32+(this.level-1)/30);
  for(let gridTry=0;gridTry<12;gridTry++){
   this.grid=Array.from({length:this.size},()=>Array(this.size).fill(''));this.placements.clear();let all=true;
   for(const obj of sorted){
    const candidates=[];
    for(let r=0;r<this.size;r++)for(let c=0;c<this.size;c++)for(const d of dirs){
     const er=r+d[1]*(obj.word.length-1),ec=c+d[0]*(obj.word.length-1);
     if(er<0||er>=this.size||ec<0||ec>=this.size)continue;
     let ok=true,overlap=0;
     for(let i=0;i<obj.word.length;i++){const ch=this.grid[r+d[1]*i][c+d[0]*i];if(ch){if(ch!==obj.word[i]){ok=false;break}overlap++}}
     if(ok)candidates.push({r,c,er,ec,d,overlap});
    }
    if(!candidates.length){all=false;break}
    for(let i=candidates.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[candidates[i],candidates[j]]=[candidates[j],candidates[i]]}
    const crossing=candidates.filter(q=>q.overlap>0);
    const pool=crossing.length&&Math.random()<crossBias?crossing:candidates;
    const q=pool[0];
    const {r,c,er,ec,d}=q;
    for(let i=0;i<obj.word.length;i++)this.grid[r+d[1]*i][c+d[0]*i]=obj.word[i];
    this.placements.set(obj.word,{r,c,er,ec});
   }
   if(all){const orientations=[...this.placements.values()].map(p=>({h:p.r===p.er&&p.c!==p.ec,v:p.c===p.ec&&p.r!==p.er}));if(this.words.length>=4&&(!orientations.some(x=>x.h)||!orientations.some(x=>x.v))){continue}const letters='ABCDEFGHIJKLMNOPQRSTUVWXYZ';for(let r=0;r<this.size;r++)for(let c=0;c<this.size;c++)if(!this.grid[r][c])this.grid[r][c]=letters[Math.floor(Math.random()*26)];this.words=sorted;return}
 ...
 bind(){
  ...
  const newSelColor=()=>{const c=SEL_COLORS[Math.floor(Math.random()*SEL_COLORS.length)];this.el.style.setProperty('--selcolor',c)};
  const move=e=>{if(!start)return;const p=this.pos(e);if(!p)return;last=p;this.clearSel();if(!this.validLine(start,p))return;const dr=Math.sign(p.r-start.r),dc=Math.sign(p.c-start.c);let r=start.r,c=start.c;for(let i=0;i<=Math.max(Math.abs(p.r-start.r),Math.abs(p.c-start.c));i++){const cell=this.cell(r,c);if(cell&&!cell.classList.contains('found'))cell.classList.add('sel');r+=dr;c+=dc}};
  const up=e=>{if(!start)return;const p=last||this.pos(e);if(p&&this.validLine(start,p))this.check(start,p);start=last=null;this.clearSel();try{this.el.releasePointerCapture?.(e.pointerId)}catch{}};
  this.el.onpointerdown=e=>{e.preventDefault();newSelColor();start=this.pos(e);last=start;try{this.el.setPointerCapture?.(e.pointerId)}catch{}};
  this.el.onpointermove=e=>{e.preventDefault();move(e)};
  this.el.onpointerup=e=>{e.preventDefault();up(e)};
  this.el.onpointercancel=up;
 }
 ...
 check(a,b){if(!this.validLine(a,b))return;const dr=Math.sign(b.r-a.r),dc=Math.sign(b.c-a.c),n=Math.max(Math.abs(b.r-a.r),Math.abs(b.c-a.c))+1;let w='';for(let i=0;i<n;i++)w+=this.grid[a.r+dr*i][a.c+dc*i];const rev=w.split('').reverse().join(''),target=this.words.find(x=>{const q=x.word;return q===w||q===rev});if(target&&!this.found.has(target.word)){this.found.add(target.word);this.onFound?.(target.word);const p=this.placements.get(target.word);if(p){let r=p.r,c=p.c;const dR=Math.sign(p.er-p.r),dC=Math.sign(p.ec-p.c);for(let i=0;i<target.word.length;i++){this.cell(r,c)?.classList.add('found');r+=dR;c+=dC}}this.onUpdate()}}
