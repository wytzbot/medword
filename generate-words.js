...
async function fromGroq(category, level, count, exclude) {
  ...
  const avoidClause = excludeList.length
    ...
    : '';
  const varietyClause = ' Prefer less-obvious but real terms from different subtopics; do not keep returning the same common words such as patient, dosage, therapy, diagnosis, treatment, or infection unless the field truly requires them.';
  const prompt = `Generate ${requested} ${difficulty} single-word medical terms for the field "${category}" for a word-search puzzle at difficulty level ${level}. Use real English medical terminology appropriate to the field, and favor variety across sub-topics within the field rather than near-synonyms of each other.${avoidClause}${varietyClause} Every word must contain letters A-Z only, be 3-14 characters long, contain no spaces, hyphens, punctuation, abbreviations, or duplicates. Give each word a short, accurate student-friendly definition under 18 words.`;
  try {
    ...
    return cleaned.length >= Math.min(10, requested) ? cleaned : null;
  } catch {
    return null;
  }
}
...
export default async function handler(req, res) {
  ...
  const words = aiWords && aiWords.length >= 10 ? aiWords : fromBank(category, lvl, cnt, excludeList);
