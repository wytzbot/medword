...
function speak(text){
 ...
 try{
  ...
  u.rate=0.92;u.pitch=1.02;u.volume=0.8;
  const chooseVoice=()=>{
   const voices=window.speechSynthesis.getVoices();
   const v=voices.find(x=>/^en(-|_)/i.test(x.lang)) || voices.find(x=>/en/i.test(x.lang));
   if(v)u.voice=v;
   try{window.speechSynthesis.speak(u)}catch{}
  };
  if(window.speechSynthesis.getVoices().length) chooseVoice();
  else { window.speechSynthesis.addEventListener('voiceschanged',chooseVoice,{once:true}); setTimeout(chooseVoice,250); }
 }catch{}
}
...
export const sound={
 ...
 complete(){chime([523.25,659.25,783.99,1046.5],{dur:.22,gap:.12,gain:.08});const lines=['Good!','Perfect!','Genius!','You killed it!','Excellent!','Brilliant!','Amazing!'];speak(lines[Math.floor(Math.random()*lines.length)])}, voice(text){speak(text)}
};
