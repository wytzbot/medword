...
export class Game{
 ...
 bind(){
  ...
  const move=e=>{if(!start)return;const p=this.pos(e);if(!p)return;last=p;this.clearSel();if(!this.validLine(start,p))return;const dr=Math.sign(p.r-start.r),dc=Math.sign(p.c-start.c);let r=start.r,c=start.c;for(let i=0;i<=Math.max(Math.abs(p.r-start.r),Math.abs(p.c-start.c));i++){this.cell(r,c)?.classList.add('sel');r+=dr;c+=dc}};
