...
export async function requestWords(category,level){
 const recent=new Set(store.getRecent(category).map(x=>String(x).toUpperCase()));
 const usable=a=>cleanTerms(a,category).filter(x=>!recent.has(x.word));
 const cached=usable(store.getTerms(category));
 if(cached.length>=6){const chosen=choose(cached,level);store.setRecent(category,chosen.map(x=>x.word));return chosen;}
 try{
  ...
  if(r.ok){
   ...
   if(terms.length>=6){store.setTerms(category,terms);const chosen=choose(usable(terms),level);store.setRecent(category,chosen.map(x=>x.word));return chosen}
  }
 }catch{}
